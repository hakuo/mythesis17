...
std::vector<pattern_t> TextObjectTTS::regexPattern = {
        ...
        {(char *)"(NGÀY )?([0-9]{1,2})-([0-9]{1,2})-([0-9]{4}) ?", &normalize_date_1},
        {(char *)"(NGÀY )?([0-9]{1,2})/([0-9]{1,2})/([0-9]{4}) ?", &normalize_date_1},
        {(char *)"(NGÀY )?([0-9]{1,2})[.]([0-9]{1,2})[.]([0-9]{4}) ?", &normalize_date_1},
        {(char *)"(TỪ |NGÀY )([0-9]{1,2})[ ]?(-|ĐẾN)[ ]?([0-9]{1,2})(\\.|\\/)([0-9]{1,2} ?)", &normalize_date_2},
        {(char *)"(TỪ |THÁNG )([0-9]{1,2})[ ]?(-|ĐẾN)[ ]?([0-9]{1,2})(\\.|\\/)([0-9]{4}) ?", &normalize_date_3},
        {(char *)"(THÁNG )?([0-9]{1,2})(\\.|-|\\/)([0-9]{4}) ?", &normalize_date_4},
        {(char *)"(NGÀY |SÁNG |TRƯA |CHIỀU |TỐI |ĐÊM |KHUYA |HAI |BA |TƯ |NĂM |SÁU |BẢY |NHẬT |QUA |NAY )([0-9]{1,2})[\\.\\/-]([0-9]{1,2}) ?", &normalize_date_5},
        {(char *)"(THỨ |HẠNG |GIẢI )([0-9]+)",&normalize_rank},
        {(char *)"([0-9]+) ?(KM/H|M/S)", &normalize_speed},
        {(char *)"([0-9]{1,2})(H|G|:)([0-9]{1,2})?( AM| PM)? ?", &normalize_time},
        {(char *)"([0-9]+)[ ]?(KG|G|MG|KM|M|CM|MM|UM|NM|HA)(2|3)?[ |.|,|:|;]", &normalize_size},
        {(char *)"([0-9]+) O C",&normalize_degree},
        {(char *)"([0-9]+)/([0-9]+)",&normalize_proportion},
        {(char *)"([0-9]+),([0-9]+) ?", &normalize_numcomma},
        {(char *)"[0-9](\\.)[0-9]", &normalize_numdot},
        {(char *)"(\\$)?([0-9]+)(\\$|%)? ?", &normalize_snum},
...
// CuongDH8
// search and replace group text from regexPattern format
void TextObjectTTS::regexHandle(std::string *src)
{
    int reti;
    regex_t regex;
...
// CuongDH8: remove "char * pattern" in prototype => unuse
